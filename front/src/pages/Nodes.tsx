...
export const Nodes = () => {
  ...
  const validateLoader = (type: string, nodeId: string, text: string) => {
    if (typeLoader === type && loader === "ON" && currentNodeId === nodeId) {
      return <Loader />;
    }
    return text;
  };
                      ...
                      <tr key={node.Id}>
                        ...
                        <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500 flex w-60 flex-wrap justify-center items-center gap-3 m-auto">
                          <button
                            ...
                            onClick={() => startNode(node.Id, "UP")}
                          >
                            {validateLoader("UP", node.Id, "UP")}
                          </button>
                          <button
                            className="bg-red-700 border-2 border-[#32e4f0] text-[#32e4f0] h-11 w-24"
                          ...
                          <button
                            className="bg-[#32e4f0] w-24 h-11 text-white"
                            onClick={() => stopNode(node.Id, "DOWN")}
                          >
                            {validateLoader("DOWN", node.Id, "DOWN")}
                          </button>
                          <button
                            className="bg-[#155163] border-2 border-[#32e4f0] text-[#32e4f0] h-11 w-24"
                            onClick={() => restartNode(node.Id, "RESTART")}
                          >
                            {validateLoader("RESTART", node.Id, "RESTART")}
                          </button>
                        </td>
                      </tr>
                    ))}
                  ...
                  {errorAPI && (
                    <tfoot className="divide-y divide-gray-200 bg-white">
                      <tr>
                        ...
                        >
                          {errorAPI.error} {errorAPI.details}
                        </td>
                      </tr>
                    </tfoot>
                  )}
